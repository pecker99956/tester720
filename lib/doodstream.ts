...
class Streamwish {
    ...
    constructor(
        { baseUrl, key }: StreamwishProps = {
            baseUrl: undefined,
            key: undefined,
        }
    ) {
        baseUrl = baseUrl || STREAMWISH_BASE_URL;
        key = key || STREAMWISH_API_KEY;

        if (!baseUrl) throw new Error("Streamwish Base URL not set");
        if (!key) throw new Error("Streamwish Key not set");
...
const doodstream = new Streamwish();
